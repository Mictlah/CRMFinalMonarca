...
export default function CamionesPage() {
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          console.log(
            "Tabla marcas_camiones no existe, usando marcas por defecto"
          );
  ...
  const guardarMarca = async () => {
    if (!marcasTableExists) {
      alert(
        "La tabla de marcas no existe. Por favor ejecuta el script de migración de base de datos."
      );
      return;
    }
  ...
  const crearRecordatorioVerificacion = async (
    camionId: string,
    camionNumero: string
  ) => {
    if (!formData.ultima_verificacion || !formData.frecuencia_verificacion)
      return;
  ...
  const crearRecordatorioSeguroMexicano = async (
    camionId: string,
    camionNumero: string
  ) => {
    ...
    try {
      ...
      const recordatorio = {
        ...
        descripcion: `La póliza de seguro mexicano ${
          formData.poliza_seguro_mexicano
  ...
  const crearRecordatorioSeguroAmericano = async (
    camionId: string,
    camionNumero: string
  ) => {
    ...
    try {
      ...
      const recordatorio = {
        ...
        descripcion: `La póliza de seguro americano ${
          formData.poliza_seguro_americano
        } vence el ${fechaVencimiento.toLocaleDateString()}`,
        fecha_vencimiento: fechaAviso.toISOString(),
    ...
    if (formData.numero_serie && formData.numero_serie.trim()) {
      try {
        const { data: camionConMismoSerie, error: errorSerie } = await supabase
          ...
          .neq(
            "id",
            editingCamion?.id || "00000000-0000-0000-0000-000000000000"
          )
          .limit(1);
        ...
        if (errorSerie) {
          ...
          if (
            errorSerie.message.includes("numero_serie") &&
            errorSerie.message.includes("does not exist")
          ) {
            console.warn(
              "Columna numero_serie no existe. Ejecuta el script de migración."
            );
          } else {
    ...
    try {
      ...
      const datosAdicionales = {
        ...
        numeros_adicionales: formData.numeros_adicionales.filter(
          (item) => item.nombre.trim() && item.numero.trim()
        ),
      };
      ...
      if (editingCamion) {
        ...
        if (error) {
          ...
          if (error.message.includes("observaciones")) {
            ...
            if (errorSinObservaciones) {
              alert(
                "Error al actualizar camión. Por favor, ejecuta el script de migración de base de datos."
              );
              return;
            } else {
              alert(
                "Camión actualizado (algunos campos adicionales no se guardaron - se requiere migración de BD)"
              );
              errorOcurrido = true;
            }
          } else {
        ...
        if (error) {
          ...
          if (error.message.includes("observaciones")) {
            const { observaciones, ...camionDataSinObservaciones } = camionData;
            const { data: dataSinObservaciones, error: errorSinObservaciones } =
              await supabase
                .from("camiones")
            ...
            if (errorSinObservaciones) {
              alert(
                "Error al crear camión. Por favor, ejecuta el script de migración de base de datos."
              );
              return;
            } else {
              alert(
                "Camión creado (algunos campos adicionales no se guardaron - se requiere migración de BD)"
              );
      ...
      if (!editingCamion) {
        await crearRecordatorioVerificacion(
          camionId,
          formData.numero_economico
        );
        await crearRecordatorioSeguroMexicano(
          camionId,
          formData.numero_economico
        );
        await crearRecordatorioSeguroAmericano(
          camionId,
          formData.numero_economico
        );
      }
      ...
      if (!errorOcurrido) {
        alert(
          editingCamion
            ? "Camión actualizado exitosamente"
            : "Camión creado exitosamente"
        );
      }
  ...
  const guardarKilometraje = async () => {
    ...
    try {
      const kilometrajeAgregado = Number.parseInt(
        kilometrajeFormData.kilometraje_agregado
      );
      const nuevoKilometraje =
        selectedCamionKilometraje.kilometraje + kilometrajeAgregado;
  ...
  const descargarExcelCamion = (camion: Camion) => {
    ...
    if (datosAdicionales.fecha_vencimiento_seguro) {
      csvContent.push(
        `"Vencimiento Seguro","${new Date(
          datosAdicionales.fecha_vencimiento_seguro
        ).toLocaleDateString()}"`
      );
    }
            ...
            {!selectedCamionKilometraje ? (
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="kilometraje_agregado">
                      ...
                      {kilometrajeFormData.kilometraje_agregado && (
                        <p className="text-sm text-green-600">
                          ...
                          {(
                            ...
                            Number.parseInt(
                              kilometrajeFormData.kilometraje_agregado || "0"
                            )
                          ).toLocaleString()}{" "}
                          km
                        </p>
                ...
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        cargarHistorialKilometraje(camionDetalle.id)
                      }
                ...
                <CardContent>
                  {loadingHistorial ? (
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">
                        Cuando captures kilometraje de esta unidad, se mostrarán
                        aquí los kilómetros capturados, la ruta y comentarios
                      </p>
                ...
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        cargarHistorialMantenimiento(camionDetalle.id)
                      }
                ...
                <CardContent>
                  {!registrosMantenimientoTableExists && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                      <div className="flex items-center space-x-2 text-yellow-800">
                        ...
                        <div>
                          <p className="font-medium">
                            Tabla de registros de mantenimiento no encontrada
                          </p>
                          <p className="text-sm">
                            Ejecuta el script de migración para crear la tabla y
                            habilitar el almacenamiento de registros.
                  ...
                  {loadingHistorialMantenimiento ? (
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">
                        Cuando registres mantenimientos de esta unidad, se
                        mostrarán aquí con todos los detalles
                      </p>
                      <p className="text-xs mt-2 text-gray-400">
                        Incluye fecha, tipo, detalles del trabajo y próximo
