...
import { Truck, Users, Search, Eye, UserCheck, AlertTriangle, Settings, Link, CheckCircle } from "lucide-react"
...
export default function AsignarOperadoresPage() {
  ...
  // Estados para el link de fotos
  const [linkCopiado, setLinkCopiado] = useState<{ [key: string]: boolean }>({})
  const [fotosEmbarques, setFotosEmbarques] = useState<{ [key: string]: FotoEmbarque[] }>({})
  ...
  // Nueva función para generar y copiar el link de subida de fotos
  const generarLinkSubidaFotos = async (embarqueId: string) => {
    try {
      const baseUrl = window.location.origin
      const linkSubida = `${baseUrl}/subir-fotos-embarque/${embarqueId}`

      await navigator.clipboard.writeText(linkSubida)

      setLinkCopiado((prev) => ({ ...prev, [embarqueId]: true }))

      // Resetear el estado después de 3 segundos
      setTimeout(() => {
        setLinkCopiado((prev) => ({ ...prev, [embarqueId]: false }))
      }, 3000)
    } catch (error) {
      console.error("Error copiando link:", error)
      alert("Error al copiar el enlace. Inténtalo de nuevo.")
    }
  }

  // Función para cargar fotos de un embarque específico
  const cargarFotosEmbarque = async (embarqueId: string) => {
    try {
      const fotos = await obtenerFotosEmbarque(embarqueId)
      setFotosEmbarques((prev) => ({ ...prev, [embarqueId]: fotos }))
    } catch (error) {
      console.error("Error cargando fotos del embarque:", error)
    }
  }
...
`
  ...
  // Componente para mostrar las fotos del embarque
  const FotosEmbarque = ({ embarqueId }: { embarqueId: string }) => {
    ...
    useEffect(() => {
      const cargarFotos = async () => {
        try {
          const fotosData = await obtenerFotosEmbarque(embarqueId)
          setFotos(fotosData)
        } catch (error) {
    ...
    if (fotos.length === 0) {
      return (
        <div className="text-center py-8">
          <p className="text-gray-500 mb-4">No hay fotos subidas para este embarque</p>
          <p className="text-sm text-gray-400">
            Las fotos aparecerán aquí una vez que el operador las suba usando el enlace proporcionado
          </p>
        </div>
      )
    }
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {fotos.map((foto, index) => (
            <div key={foto.id} className="bg-white border rounded-lg p-3">
              <img
                src={foto.url_blob || "/placeholder.svg"}
                alt={foto.nombre_archivo}
                className="w-full h-32 object-cover rounded-md mb-2"
                onError={(e) => {
                  e.currentTarget.src = "/placeholder.svg?height=128&width=128&text=Error+cargando+imagen"
                }}
              />
              <div className="space-y-1">
                <p className="text-xs font-medium text-gray-900 truncate">{foto.nombre_archivo}</p>
                <p className="text-xs text-gray-500">Subida por: {foto.subido_por || "Desconocido"}</p>
                <p className="text-xs text-gray-400">
                  {new Date(foto.fecha_subida).toLocaleDateString()} {new Date(foto.fecha_subida).toLocaleTimeString()}
                </p>
                {foto.tamano_bytes && (
                  <p className="text-xs text-gray-400">{(foto.tamano_bytes / 1024 / 1024).toFixed(2)} MB</p>
                )}
              </div>
            </div>
          ))}
                      ...
                      {(embarque.estado === "asignado" || embarque.estado === "en-transito") && (
                        <>
                          <Button
                            ...
                            size="sm"
                            className={`${
                              linkCopiado[embarque.id]
                                ? "bg-green-50 border-green-200 text-green-700"
                                : "bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100"
                            }`}
                            onClick={() => generarLinkSubidaFotos(embarque.id)}
                          >
                            {linkCopiado[embarque.id] ? (
                              <>
              ...
              <CardContent className="space-y-4">
                ...
                <div
                  className={`rounded-lg border-2 ${
                    embarque.modificado ? "border-red-300 bg-red-50" : "border-gray-200 bg-white"
                  }`}
                >
                ...
                <div className="border-b border-gray-200 mb-6">
                  <nav className="flex space-x-8" aria-label="Tabs">
                    ...
                    <button
                      ...
                      onClick={() => {
                        setActiveTab("fotos")
                        cargarFotosEmbarque(embarqueDetalle.id)
                      }}
                    >
                      Archivos Adjuntos
                    </button>
                  ...
                  {activeTab === "fotos" && (
                    <div className="space-y-6">
                      <div className="bg-white border rounded-lg p-6">
                        ...
                        <FotosEmbarque embarqueId={embarqueDetalle.id} />
                      </div>
                    </div>
                  )}
